...
public  class DBCont {
    ...
    public void insert(String cName,int cTwenty,String cDate){
        Calendar c = Calendar.getInstance();
        int year = c.get(Calendar.YEAR);
        int month = c.get(Calendar.MONTH);
        int day =c.get(Calendar.DAY_OF_MONTH);
        String createDate = String.valueOf(year) + "-" + String.valueOf(month + 1) + "-" + String.valueOf(day);
        String temp =String.format("INSERT INTO contracaption (cName,cTwenty,cDate,createDate) VALUES  ('%s',%s,'%s','%s')",cName,cTwenty,cDate,createDate);
        database.execSQL(temp);
    }

    public void update(String cName,int cTwenty,String cDate,int cid){

        String temp =String.format("update contracaption set cName='%s',cTwenty=%s,cDate='%s'  where cid= %s" ,cName,cTwenty,cDate,cid);
        database.execSQL(temp);
    }

    public Contracaption.mValue selectAllData(int id) {

        String strSQL = String.format("SELECT cid,cName,cTwenty,cDate,createDate FROM contracaption where cid=%s",id );
